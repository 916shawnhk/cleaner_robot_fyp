...
def generate_launch_description():
    ...
    teleop = Node(
        ...
        output='screen',
        prefix='xterm -e',
        # CHANGED: publish directly to /cmd_vel (Gazebo DiffDrive plugin topic)
        # REMOVED: old remapping to /teleop/cmd_vel + twist_stamper relay
    )

    return LaunchDescription([
        teleop,
        # REMOVED: relay node (twist_stamper no longer needed)
    ])